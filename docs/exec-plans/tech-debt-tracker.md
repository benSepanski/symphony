...
| 2026-04-18 | web         | No jsdom + `@testing-library/react` suite. Extracted `*Utils.ts` helpers cover branching logic; component render + user-interaction behavior still relies on API layer + manual QA. | med      | `src/web/`                  | Add `@testing-library/react` + `jsdom`; pin the four routes.                                                     |
