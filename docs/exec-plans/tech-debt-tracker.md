...
| 2026-04-18 | architecture | Layer-direction rule is review-only; no automatic graph check.                                     | med      | `tests/` (new)              | Write an import-graph test that asserts forward-only edges across Types / Config / Persistence / Service / Runtime / API. |
...
| 2026-04-18 | ops          | Leftover `worktrees/BEN-*` from old Elixir runs — destructive to clean up.                         | low      | repo root                   | Decide keep-as-archive vs. `git worktree remove && git branch -D`.                                                        |
